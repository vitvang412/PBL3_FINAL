...
namespace DaNangSafeMap.Repositories
{
    ...
    public interface IAlertRepository
    {
        ...
        // ── Auto-expire (NV8) ──
        Task<List<SecurityAlert>> GetExpiredAlertsAsync(bool includeNeedsMoreInfoDeadlineExpiration);

        // ── Clustering check (NV4) ──
        Task<int> CountNearbyAlertsAsync(decimal lat, decimal lng, int alertTypeId, int withinMinutes);
        Task<int> CountRecentReportsByUserAsync(int userId, TimeSpan within);
        Task<int> CountRejectedReportsByUserAsync(int userId, TimeSpan within);
